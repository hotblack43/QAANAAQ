...
parameters {
  real<lower=0, upper=8000> easting_source;  // source easting
  real<lower=5000, upper=9000> northing_source;  // source northing
  real<lower=0, upper=100> height_source;    // source height
  real<lower=-500, upper=0> t0;              // time offset
}
...
stan_fit <- stan(model_code = stan_model, data = stan_data, 
                 iter = 4*12000, warmup = 4000, chains = 8, thin = 50, control = list(max_treedepth = 20))
