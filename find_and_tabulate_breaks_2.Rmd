...
speed_calc <- function(df_her) {
  ...
  for (itime in 1:(nrow(df_her) - 1)) {
    distance <- distHaversine(cbind(df_her$Longitude[itime], df_her$Latitude[itime]), 
                              ...
                              r = 6378137)
    speed[itime] <- abs(distance / time_diff[itime]) # m/sec
    
    # Debugging information (optional)
    #print(paste("DateTimeUTC:", df_her$DateTimeUTC[itime]))
    #print(paste("Distance (meters):", distance))
    #print(paste("Time difference (seconds):", time_diff[itime]))
    #print(paste("Speed (m/sec):", distance / time_diff[itime]))
...
findbreaks <- function(df_in,main_str)
{
  ...
  speed <- abs(speed_calc(df_in))
  speed[nrow(df_in)] <- NA

  plot(df_in$DateTimeUTC,speed,log="y",pch=19,cex=0.3,ylab="Speed [m/sec]",main=main_str)
