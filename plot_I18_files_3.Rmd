...
```{r}
gohipassfilter <- function(data, window_size)
{
  # Create a low-pass filter using a moving average
  low_pass_filter <- filter(data, rep(1 / window_size, window_size), sides =
                              2)
...
for (window_start in window_start_times) {
  ...
  for (icol in 2:(ncol(dd) - 1)) {
    dd[, icol] <- gohipassfilter(dd[, icol], window_size)
  }
  dd <- na.omit(dd)
  if (nrow(dd) < (window_length / 2)) {
...
print(results)
# Save it
saveRDS(results,file=paste0('OUTPUT/sliding_window_',window_size,'_delays_data_2022123.rds'))
```
