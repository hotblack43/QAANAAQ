...
for (ifil in 3:3) # 1:length(files))
{
  ...
  par(mfrow=c(1,1))
  #----------
  #data <- scale(df[,c(5,3,2)]) # use time,lon,lat
  data <- scale(df[,c(6,3,2)]) # use speed,lon,lat
  modl <- Mclust(data,G=Groups_wanted) 
#  plot(modl, what = "BIC")
  cluster_assignments <- modl$classification
  cluster_probabilities <- modl$z
...
# Determine date range of each class
  ...
  # Plot Latitude
#  par(mfrow=c(2,1))
    # Plot Latitude vs Longitude with colored segments
